...
L'image est taguée:
- `ghcr.io/etho01/cinegest-front:${GITHUB_SHA}`
- `ghcr.io/etho01/cinegest-front:latest`
...
# Logs
kubectl -n cinegest-site logs -l app=cinegest-front
...
2. Vérifier l'ExternalSecret:
   ```bash
   kubectl -n cinegest-site describe externalsecret
   ```
3. Vérifier le SecretStore:
   ```bash
   kubectl -n cinegest-site describe secretstore vault-frontend
   ```
...
```bash
kubectl -n cinegest-site rollout undo deployment/cinegest-front
```
...
```bash
# Watch pods
kubectl -n cinegest-site get pods -w

# Logs en temps réel
kubectl -n cinegest-site logs -f -l app=cinegest-front
